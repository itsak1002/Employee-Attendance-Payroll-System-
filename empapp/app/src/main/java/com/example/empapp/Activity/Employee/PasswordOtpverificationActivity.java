...
public class PasswordOtpverificationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        verifyOtpButton.setOnClickListener(view -> {
            ...
            if (isOtpValid && enteredOtp.equals(generatedOtp)) {
                ...
                Toast.makeText(PasswordOtpverificationActivity.this, "OTP Verified Successfully!", Toast.LENGTH_SHORT).show();

                // Navigate to the dashboard after 2 seconds
                new Handler(Looper.getMainLooper()).postDelayed(() -> {
                    Intent intent = new Intent(PasswordOtpverificationActivity.this, ChangePasswordActivity.class);
                    intent.putExtra("employeeId", employeeId);  // Pass the employee ID to the dashboard
                    startActivity(intent);
                    finish();  // Close the current activity
                }, 2000);  // 2 seconds delay before navigating to dashboard
            } else {
                Toast.makeText(PasswordOtpverificationActivity.this, "Invalid or Expired OTP. Try Again.", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void sendOtp(String otp) {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.SEND_SMS) == PackageManager.PERMISSION_GRANTED) {
            ...
            String message = "Your OTP for Change Password of employee ID  " + employeeId + " OTP is: " + otp + " in Smart Technology";
    ...
    private void startOtpTimeout() {
        new Handler().postDelayed(() -> {
            isOtpValid = false;
            Toast.makeText(PasswordOtpverificationActivity.this, "OTP has expired. Please request a new one.", Toast.LENGTH_SHORT).show();
        }, OTP_TIMEOUT_MS);
    }
    ...
    private void startResendOtpTimer() {
        ...
        new CountDownTimer(OTP_RESEND_INTERVAL_MS, 1000) {
            ...
            public void onFinish() {
                ...
                resendOtpButton.setBackgroundTintList(ContextCompat.getColorStateList(PasswordOtpverificationActivity.this, R.color.colorPrimary));
            }
        }.start();
    }
